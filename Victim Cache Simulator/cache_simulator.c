...
void write_cache (Cache *cache1,Cache *vicache1, Results *result1, int index1, long unsigned line1, int data1, int associativity, char *replacement_policy) {
    /** Writing the data in the set (by index) in the position that contains the
      *     upper (by line).
      */
    char *lru  = "LRU\0";
    char *fifo = "FIFO\0";
    int is_full,is_fullv;
    int rpl, rpf;
    int free_block,free_blockv;
    int p_lAc, p_lLd,p_lAcv, p_lLdv;     // Position of the less counter for LRU(l) and FIFO(f)
    ...
    if (position_that_has_this_upper == -1) { // position with the right upper was not found
        ...
        int position_that_has_this_upperv = getPosUpper(vicache1, 0, line1, 64);
    /****************************** Write Victim Miss ********************************/
        if(position_that_has_this_upperv == -1){
		result1->write_missesv++; // (*result1).write_misses++  this points to the cache_mem in the main
			...
			is_fullv = there_Are_Space_Set(vicache1, 0, 64);
			if(is_fullv == 1){
			/*************************** Victim Set is full *****************************/
				p_lAcv = findLessAccessTSset(vicache1, 0, 64);
				
				vicache1->Cache_Upper[0][p_lAcv] = cache1->Cache_Upper[index1][p_lAc]; // line1 = upper
	                	vicache1->Cache_Data[0][p_lAcv] = cache1->Cache_Data[index1][p_lAc];
	                	vicache1->T_Access[0][p_lAcv] = currently_clk;  // Update T_Access
	                	vicache1->T_Load[0][p_lAcv]   = currently_clk ; // Update T_Load
			}
			else {                  // There are a free block in the set (by index1)
            			free_blockv = random_free_space_set (vicache1, 0, 64);

            			vicache1->Cache_Upper[0][free_blockv] = cache1->Cache_Upper[index1][p_lAc];
            			vicache1->Cache_Data[0][free_blockv] = cache1->Cache_Data[index1][p_lAc];
            			vicache1->T_Access[0][free_blockv] = currently_clk; // Update the T_Access
            			vicache1->T_Load[0][free_blockv] = currently_clk;   // Update the T_Load
        		}
			...
			is_fullv = there_Are_Space_Set(vicache1, 0, 64);
			if(is_fullv == 1){
				p_lLdv = findLessLoadTSset(vicache1, 0, 64);
				
				vicache1->Cache_Upper[0][p_lLdv] = cache1->Cache_Upper[index1][p_lLd]; // line1 = upper
	                	vicache1->Cache_Data[0][p_lLdv] = cache1->Cache_Data[index1][p_lLd];
	                	vicache1->T_Access[0][p_lLdv] = currently_clk;  // Update T_Access
	                	vicache1->T_Load[0][p_lLdv]   = currently_clk ; // Update T_Load
			}
			else {                  // There are a free block in the set (by index1)
            			free_blockv = random_free_space_set (vicache1, 0, 64);

            			vicache1->Cache_Upper[0][free_blockv] = cache1->Cache_Upper[index1][p_lLd];
            			vicache1->Cache_Data[0][free_blockv] = cache1->Cache_Data[index1][p_lLd];
            			vicache1->T_Access[0][free_blockv] = currently_clk; // Update the T_Access
            			vicache1->T_Load[0][free_blockv] = currently_clk;   // Update the T_Load
        		}

	                cache1->Cache_Upper[index1][p_lLd] = line1; // line1 = upper
	...
	/*************************** Set is not full *****************************/
		...
		result1->write_hitsv++;
	        ...
	        if (is_full == 1) {
	            ...
	            if (rpl == 0) { // It's LRU
           		...
           		vicache1->Cache_Upper[0][position_that_has_this_upperv] = cache1->Cache_Upper[index1][p_lAc];
            		vicache1->Cache_Data[0][position_that_has_this_upperv] = cache1->Cache_Data[index1][p_lAc];
            		vicache1->T_Access[0][position_that_has_this_upperv] = currently_clk; // Update the T_Access
            		vicache1->T_Load[0][position_that_has_this_upperv] = currently_clk;   // Update the T_Load
	            ...
	            if (rpf == 0) { // It's FIFO
            		...
            		vicache1->Cache_Upper[0][position_that_has_this_upperv] = cache1->Cache_Upper[index1][p_lLd];
            		vicache1->Cache_Data[0][position_that_has_this_upperv] = cache1->Cache_Data[index1][p_lLd];
            		vicache1->T_Access[0][position_that_has_this_upperv] = currently_clk; // Update the T_Access
            		vicache1->T_Load[0][position_that_has_this_upperv] = currently_clk;   // Update the T_Load
		...
		vicache1->Cache_Upper[0][position_that_has_this_upperv] = 0;
            	vicache1->Cache_Data[0][position_that_has_this_upperv] = 0;
            	vicache1->T_Access[0][position_that_has_this_upperv] = 0; // Update the T_Access
            	vicache1->T_Load[0][position_that_has_this_upperv] = 0;   // Update the T_Load
        	}
	}
    }
    /**************************************************************************/
...
void read_cache (Cache *cache1,Cache *vicache1, Results *result1, int index1, long unsigned line1, int data1, int associativity, char *replacement_policy) {
...
void generate_output(Results cache_results, char *output_name){
    ...
    if (!ptr_file_output) {
        ...
        fprintf(ptr_file_output, "Total Access count:%d\n", cache_results.acess_count);
        fprintf(ptr_file_output, "L1 Read hits:%d\n", cache_results.read_hits);
        fprintf(ptr_file_output, "L1 Read misses:%d\n", cache_results.read_misses);
        fprintf(ptr_file_output, "L1 Write hits:%d\n", cache_results.write_hits);
        fprintf(ptr_file_output, "L1 Write misses:%d\n", cache_results.write_misses);
	fprintf(ptr_file_output, "Victim Read hits:%d\n", cache_results.read_hitsv);
        fprintf(ptr_file_output, "Victim Read misses:%d\n", cache_results.read_missesv);
        fprintf(ptr_file_output, "Victim Write hits:%d\n", cache_results.write_hitsv);
        fprintf(ptr_file_output, "Victim Write misses:%d\n", cache_results.write_missesv);
	fprintf(ptr_file_output, "percentage increase in hit:%f\n", ((double)(cache_results.write_hitsv+cache_results.read_hitsv)/(double)(cache_results.write_hits+cache_results.read_hits))*100);
        fprintf(ptr_file_output, "Hit Rate:%f\n", (double)(cache_results.read_hits+cache_results.write_hits+cache_results.write_hitsv+cache_results.read_hitsv)/(double)cache_results.acess_count);
    }
...
int main(int argc, char **argv)               // Files are passed by a parameter
{
    ...
    Cache   cache_mem,vicache_mem;   // This contains the data, access and load informations
    ...
    cache_results.read_hitsv = 0;
    cache_results.read_missesv = 0;
    cache_results.write_hitsv = 0;
    cache_results.write_missesv = 0;
    ...
    vicache_mem.Cache_Data = malloc(1* sizeof(int *));
    for (i=0; i<1; i++) {
        vicache_mem.Cache_Data[i] = malloc (64 * sizeof(int));
    }
    /**************************************************************************/
    ...
    vicache_mem.Cache_Upper = malloc( 1 * sizeof(long unsigned *));
    for (i=0; i<1; i++) {
        vicache_mem.Cache_Upper[i] = malloc (64 * sizeof(long unsigned));
    }
    /**************************************************************************/
    ...
    vicache_mem.T_Access = malloc( 1 * sizeof(long unsigned *));
    for (i=0; i<1; i++) {
        vicache_mem.T_Access[i] = malloc (64 * sizeof(long unsigned));
    }
    /**************************************************************************/
    ...
    vicache_mem.T_Load = malloc( 1 * sizeof(long unsigned *));
    for (i=0; i<1; i++) {
        vicache_mem.T_Load[i] = malloc (64 * sizeof(long unsigned));
    }
    /**************************************************************************/

    startCache(&cache_mem, number_of_sets, cache_description.associativity);
    startCache(&vicache_mem, 1, 64);
    ...
    if (!ptr_file_input) {
        ...
        while (fscanf(ptr_file_input, "%lu %c\n", &address, &RorW) != EOF){
            ...
            if (RorW == 'R') {
                ...
                number_of_reads++;
                read_cache(&cache_mem,&vicache_mem, &cache_results, index, line, data, cache_description.associativity, cache_description.replacement_policy);
            }
            else if (RorW == 'W'){
                line  = make_upper(address, BYTES_PER_WORD, words_per_line);
                index = make_index (number_of_sets, line);
                number_of_writes++;
                write_cache(&cache_mem,&vicache_mem, &cache_results, index, line, data, cache_description.associativity, cache_description.replacement_policy);
            }
            else {
    ...
    vicache_mem.Cache_Data  = NULL;  // "Free" in the memory for the Cache_Data[][]
    vicache_mem.Cache_Upper = NULL;  // "Free" in the memory for the Cache_Upper[][]
    vicache_mem.T_Access    = NULL;  // "Free" in the memory for the T_Access[][]
    vicache_mem.T_Load      = NULL;  // "Free" in the memory for the T_Load[][]
