/**
  * --------------------------------------------------------------------------
  * Skewed Associative Cache Simulator
  * --------------------------------------------------------------------------
  */
#include <unistd.h>
// Libraries
 ...
 */
int make_index1 (int number_of_sets, long unsigned upper) {
    int index;
    //printf("NUMBER OF SETS: %d\n", number_of_sets);
    //printf("UPPER: %lu\n", upper);
    long unsigned l1,l2,t;
    l1 = upper / number_of_sets;
    l2 = l1 % number_of_sets;
    l1 = upper % number_of_sets;
    t = 0;
    for(int i=1;powe(2,i)<=number_of_sets;i=i+2)
	t = t + powe(2,i);
    t=powe(2,number_of_sets) - t;
    index = l2 ^ (l1 & t); 
    return index;                                  // index of the set in the cache
}
int make_index2 (int number_of_sets, long unsigned upper) {
    int index;
    //printf("NUMBER OF SETS: %d\n", number_of_sets);
    //printf("UPPER: %lu\n", upper);
    long unsigned l1,l2,t;
 ...
 */
int getPosUpper (Cache *cache, int index1, int index2, long unsigned line, int associativity) {
    
    //printf("INDEX POS UPPER %d, ASSOC: %d, LINE: %lu\n", index, associativity, line);
        if (cache->Cache_Upper[index1][0] == line)
            return 0; 
	if (cache->Cache_Upper[index2][1] == line)
            return 1;               
...
/** This function verifies if are or aren't free blocks (lines) available in
  ...
  */
int there_Are_Space_Set(Cache *cache1, int index1, int index2, int associativity) {
    
        if (cache1->Cache_Data[index1][0]==0) 
		return 0;
	if(cache1 ->Cache_Data[index2][1]==0)    
		return 1;
	return -1;
}
...
/** This function gives a block (line) free in a set (by index1) of the cache
  ...
  */
int random_free_space_set (Cache *cache1, int index1, int index2, int associativity) {
...
int findLessAccessTSset (Cache *cache1, int index1, int index2, int associativity) {
     // Position when is the less T_Access in the set (by index)
    long unsigned lessAc1 = cache1->T_Access[index1][0];
    long unsigned lessAc2 = cache1->T_Access[index2][1];
	if( lessAc1>lessAc2 )
	   return 1;
    return 0;
}

int findLessLoadTSset (Cache *cache1, int index1, int index2, int associativity) {
    long unsigned lessLd1 = cache1->T_Load[index1][0];
    long unsigned lessLd2 = cache1->T_Load[index2][1];
	if( lessLd1>lessLd2 )
	   return 1;
    return 0;
}

void write_cache (Cache *cache1, Results *result1, int index1, int index2, long unsigned line1, int data1, int associativity, char *replacement_policy) {
    ...
    int position_that_has_this_upper = getPosUpper(cache1, index1, index2, line1, associativity);
    ...
    if (position_that_has_this_upper == -1) { // position with the right upper was not found
        ...
        /** It is necessary to know if the set is full (it will use FIFO or LRU
          ...
          */
        is_full = there_Are_Space_Set(cache1, index1, index2, associativity);
        ...
        if (is_full == -1) {
            ...
            if (rpl == 0) { // It's LRU
                p_lAc = findLessAccessTSset(cache1, index1, index2, associativity);
		if(p_lAc == 0){
		...
		cache1->Cache_Upper[index2][p_lAc] = line1; // line1 = upper
                cache1->Cache_Data[index2][p_lAc] = DATA;
                cache1->T_Access[index2][p_lAc] = currently_clk;  // Update T_Access
                cache1->T_Load[index2][p_lAc]   = currently_clk ; // Update T_Load
		}
            }
            // *** FIFO ***
            rpf = strcmp(fifo, replacement_policy);
            if (rpf == 0) { // It's FIFO
                p_lLd = findLessLoadTSset(cache1, index1, index2, associativity);
		if(p_lLd == 0){
		...
		cache1->Cache_Upper[index2][p_lLd] = line1;
                cache1->Cache_Data[index2][p_lLd] = DATA;
                cache1->T_Access[index2][p_lLd] = currently_clk; // Update T_Access
                cache1->T_Load[index2][p_lLd] = currently_clk;   // Update T_Load
		}
            }
        ...
        /************************** Set isn't full ****************************/
            ...
            free_block = random_free_space_set (cache1, index1, index2, associativity);
	    if(free_block == 0){
	    ...
	    cache1->Cache_Upper[index2][free_block] = line1;
            cache1->Cache_Data[index2][free_block] = DATA;
            cache1->T_Access[index2][free_block] = currently_clk; // Update the T_Access
            cache1->T_Load[index2][free_block] = currently_clk;   // Update the T_Load
	    }
        }
    }
    /**************************************************************************/
    ...
    /****************************** Write Hit ********************************/
         ...
         */
        result1->write_hits++;
	if(position_that_has_this_upper == 0){
        ...
        cache1->Cache_Upper[index2][position_that_has_this_upper] = line1;
        cache1->T_Access[index2][position_that_has_this_upper] = currently_clk; // Update the T_Access
	}
    }
}

void read_cache (Cache *cache1, Results *result1, int index1, int index2, long unsigned line1, int data1, int associativity, char *replacement_policy) {
...
void generate_output(Results cache_results, char *output_name){
    ...
    if (!ptr_file_output) {
        ...
        fprintf(ptr_file_output, "Hit Rate:%f\n", (double)(cache_results.read_hits+cache_results.write_hits)/(double)cache_results.acess_count);
    }
    ...
    if (!ptr_file_input) {
        ...
        while (fscanf(ptr_file_input, "%lu %c\n", &address, &RorW) != EOF){
            ...
            if (RorW == 'R') {
                ...
                index1 = make_index1 (number_of_sets, line);
		index2 = make_index2 (number_of_sets, line);
                ...
                number_of_reads++;
                read_cache(&cache_mem, &cache_results, index1, index2, line, data, cache_description.associativity, cache_description.replacement_policy);
            }
            else if (RorW == 'W'){
                line  = make_upper(address, BYTES_PER_WORD, words_per_line);
                index1 = make_index1 (number_of_sets, line);
		index2 = make_index2 (number_of_sets, line);
                number_of_writes++;
                write_cache(&cache_mem, &cache_results, index1, index2, line, data, cache_description.associativity, cache_description.replacement_policy);
            }
            else {
